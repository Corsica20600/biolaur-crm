-- Import BIOLAUR SP 2026 CORSE adapte au schema production actuel
-- Source: BIOLAUR_import_supabase.sql

begin;

alter table public.product_categories add column if not exists slug text;
create unique index if not exists product_categories_owner_slug_unique
  on public.product_categories(owner_id, slug)
  where slug is not null;
create unique index if not exists price_lists_owner_name_unique
  on public.price_lists(owner_id, name);

-- 1) Categories principales
insert into public.product_categories (owner_id, name, slug, parent_id, created_at, updated_at)
values
((select id from public.profiles order by created_at limit 1), 'CORPOREL', 'corporel', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'CUISINE', 'cuisine', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'DEBOUCHEURS', 'deboucheurs', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'DROGUERIE', 'droguerie', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'GAMME NILODOR', 'gamme-nilodor', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'GEL DOUCHE', 'gel-douche', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'LINGE', 'linge', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'SAC A DECHET NETTOYANT POUBELLES', 'sac-a-dechet-nettoyant-poubelles', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'SANITAIRES', 'sanitaires', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'SHAMPOING', 'shampoing', null, now(), now()),
((select id from public.profiles order by created_at limit 1), 'SOLS & SURFACES', 'sols-surfaces', null, now(), now())
on conflict (owner_id, slug) where slug is not null do update set
  name = excluded.name,
...
join public.product_categories p
 ...
 and p.owner_id = (select id from public.profiles order by created_at limit 1)
...
-- 3) Liste tarifaire
insert into public.price_lists (owner_id, name, effective_date, active, created_at, updated_at)
values ((select id from public.profiles order by created_at limit 1), 'Tarif BIOLAUR SP 2026 CORSE', date '2026-01-01', true, now(), now())
...
insert into public.products (
  owner_id, categorie_id, reference, code_produit, nom_produit, description_courte,
  gamme, categorie, sous_categorie, conditionnement, unite, ean, tarif_ht, tva, actif,
  fiche_technique_url, fiche_securite_url, notes, created_at, updated_at
)
select
  (select id from public.profiles order by created_at limit 1) as owner_id,
  pc.id as categorie_id,
  v.reference,
  v.code as code_produit,
  v.name as nom_produit,
  v.short_description as description_courte,
  v.range_name as gamme,
  coalesce(parent.name, pc.name) as categorie,
  pc.name as sous_categorie,
  v.packaging as conditionnement,
  v.unit as unite,
  null as ean,
  0.00 as tarif_ht,
  20.00 as tva,
  true as actif,
  '' as fiche_technique_url,
  null as fiche_securite_url,
  v.notes,
  now(),
  now()
from (
...
left join public.product_categories pc
  on pc.slug = v.category_slug
 and pc.owner_id = (select id from public.profiles order by created_at limit 1)
left join public.product_categories parent
...
insert into public.price_list_items (
  owner_id, price_list_id, product_id, prix_ht, remise, conditionnement, disponibilite,
...
select
  ...
  p.id as product_id,
  v.unit_price_ht as prix_ht,
  0.00 as remise,
  v.conditioning as conditionnement,
  'en_stock' as disponibilite,
...
join public.products p
  on p.reference = v.reference
 and p.owner_id = (select id from public.profiles order by created_at limit 1)
join public.price_lists pl
  on pl.owner_id = (select id from public.profiles order by created_at limit 1)
 and pl.name = 'Tarif BIOLAUR SP 2026 CORSE'
...
-- Synchronise le prix visible catalogue depuis le tarif actif
update public.products p
set tarif_ht = pli.prix_ht, updated_at = now()
from public.price_list_items pli
join public.price_lists pl on pl.id = pli.price_list_id
where p.id = pli.product_id
  and pl.owner_id = (select id from public.profiles order by created_at limit 1)
  and pl.name = 'Tarif BIOLAUR SP 2026 CORSE';

-- 6) Rattachement automatique des fiches techniques si des PDF sont deja uploades
with matches as (
  select
    p.id as product_id,
    p.reference,
    ...
    'https://mrtsoahvnhclcmigungi.supabase.co/storage/v1/object/public/technical-sheets/' || replace(o.name, ' ', '%20') as public_url,
    row_number() over (partition by p.id order by length(o.name), o.name) as rn
  from public.products p
  join storage.objects o on o.bucket_id = 'technical-sheets'
  where p.owner_id = (select id from public.profiles order by created_at limit 1)
    and lower(o.name) like '%.pdf'
    and (
      lower(regexp_replace(o.name, '^.*/', '')) = lower(p.reference || '.pdf')
      or lower(regexp_replace(o.name, '^.*/', '')) like lower(p.reference || ' %')
      or lower(regexp_replace(o.name, '^.*/', '')) like lower(p.reference || '_%')
      or lower(regexp_replace(o.name, '^.*/', '')) like lower(p.reference || '-%')
    )
), inserted_docs as (
  insert into public.product_documents (product_id, document_type, title, file_name, storage_path, public_url, mime_type, created_at, updated_at)
  select
    m.product_id,
    'fiche_technique',
    'Fiche technique ' || m.reference,
    m.file_name,
    m.storage_path,
    m.public_url,
    'application/pdf',
  ...
  where m.rn = 1
    and not exists (
      ...
      where d.product_id = m.product_id
        and d.document_type = 'fiche_technique'
        and coalesce(d.storage_path, '') = m.storage_path
    )
  returning product_id, public_url
)
update public.products p
set fiche_technique_url = coalesce(m.public_url, p.fiche_technique_url), updated_at = now()
from matches m
